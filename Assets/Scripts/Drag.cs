        ...
        if (!paused.GetComponent<Pause>().isPaused)
        {
            ...
            if (e.type == EventType.MouseDown)
            {
                for (int i = 0; i < explosivesRect.Length; i++)
                {
                    if (explosivesRect[i].Contains(e.mousePosition))
                    {
                        ...
                        GetComponent<Statistics>().BufferAddExplosive();
                        cost = cost + spawn.GetComponent<Explosion>().cost;
                        spawn.GetComponent<Explosion>().sfxController.GetComponent<sfxcontroller>().AddExplosive(spawn.gameObject); //Add the Explosive to the SFX List.                 
            ...
            if ((trashButtonRect.Contains(e.mousePosition) && isTouching) && e.type == EventType.MouseUp)
            {
                if (explosive != null)
                {
                    ...
                    explosive.GetComponent<Explosion>().sfxController.GetComponent<sfxcontroller>().RemoveExplosive(explosive.gameObject);
                    Destroy(explosive);
                    GetComponent<Statistics>().BufferRemoveExplosive();
                }
                if (spawn != null)
                {
                    cost = cost - spawn.GetComponent<Explosion>().cost;
                    spawn.GetComponent<Explosion>().sfxController.GetComponent<sfxcontroller>().RemoveExplosive(spawn.gameObject);
